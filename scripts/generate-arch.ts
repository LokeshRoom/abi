...
function generateArchitectureMarkdown(): string {
...
function main() {
  console.log('Generating architecture documentation at:', archPath);
  const markdown = generateArchitectureMarkdown();
  
  const docsDir = path.dirname(archPath);
  if (!fs.existsSync(docsDir)) {
    fs.mkdirSync(docsDir, { recursive: true });
  }
  
  fs.writeFileSync(archPath, markdown, 'utf8');
  console.log('Architecture documentation successfully written.');
}
